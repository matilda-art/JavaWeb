 ...
 * 实验一： TCP 短连接  echo服务
 *       Socket：
 *          专为服务端使用的Socket  ServerSocket
 *          专为客户端使用的Socket  Socket
 *          读取/写入数据的时候，使用Java中的Stream抽象类(InputStream/OutputStream)之前讲的IO
 * @author: matilda
 * @create: 2020-06-29 19:09
 **/
public class Server {
    public static final int SERVER_PORT = 9260;//作为服务器，需要有公开端口号
    ...
    public static void main(String[] args) throws IOException {
        ...
        try (ServerSocket serverSocket = new ServerSocket(SERVER_PORT)) {
            ...
            while (true) {
                ...
                Logger.debug("准备发送响应");
                writer.printf("%s\r\n", response);
                Logger.debug("调用flush清空操作，把数据真正写入Socket中");
                writer.flush();
                Logger.debug("发送响应成功");
