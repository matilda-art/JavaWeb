...
//4.调用方法之前都需要先对相应对象加锁(sync)，并且是wait/notify的那个对象
//5.wait本身会去释放锁（释放锁到重新请求锁这段时间：放弃CPU,等待被唤醒）

//为什么调用前需要加锁？ wait/notify都会修改等待集，所以需要先加锁
//为什么要先释放，再申请锁？ 不释放的话，别的线程就没法把它唤醒了

//notify只唤醒该等待集上的一个线程，唤醒哪个不确定
//notifyAll唤醒等待集上的所有线程